...
enum struct EPlayerStat
{
        int st_kills;
        char playerName[MAX_PLAYER_NAME];
        float damageInflicted;
        float damageTaken;
        int headShots;                  // Head shot count
        int st_shots;                   // Times shot weapon
        int st_damages;                 // Times received damage
        float st_selfinflicteddmg;      // Self inflicted
        int st_sui;                     // Suicides

        int st_killstreak;              // Current kill streak counter
        int st_killstreakcount;         // How many killstreaks player has had
        int st_killstreakmax;           // Highest kill streaks

        int st_deathstreak;             // Current death streak
        int st_deathstreakcount;
        int st_deathstreakmax;          // Maximum death streak
}

enum struct ENonTrace
{
        int en_entity;
        char en_classname[30];
}


EPlayerStat playerStats[MAXPLAYERS+1];
